...
function getWeatherCandyData(request, response) {
  ...
  if("imageCategory" in request.params) {
    imageCategory = Number(request.params.imageCategory);
    console.log("imageCategory is "+imageCategory);
  } else {
    console.log("no image category in weather request");
  }
  ...
  if (cityID !== undefined ) { 
    ...
    console.error("didnt get enough info in the call, weather going to fail.");
  }
  ...
  if (imageCategory == undefined) {
    console.log("imageCategory is not set");
